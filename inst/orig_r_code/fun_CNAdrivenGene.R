...
#'
#' \code{CNAdrivenGene} returns a list of CNA-driven differentially expressed genes, including false discovery rate,
#' p-value and associated statistics.
#'
#'
...
#'
#' @param CNAtoGeneList
#'        CNAtoGeneList.rda, the output file from function CNAtoGene.
#' @param GeneExp
#'        GeneExp.rda, the output file from function ListGeneExp.
#' @param probe
#'        True if \strong{the GeneExp.rda has probe information}. \strong{Default value is FALSE}.
#' @param sample.mapping.file
#'        Filename if \strong{the sample IDs are different in CNAtoGeneList.rda and GeneExp.rda}.
#' @param directory
#'        The directory sample mapping file locates.
#' @param map.loc.start
#'        The starting position of substring of sample name.
#' @param map.loc.end
#'        The ending position of substring of sample name.
#' @param gain.ratio
#'        The number of CNA-gain samples to the number of total samples, where \strong{default value is set as 20\%}.
#' @param loss.ratio
#'        The number of CNA-loss samples to the number of total samples, where \strong{default value is set as 20\%}.
#' @param tcga
#'        True if TCGA's data is used. \strong{Default value is TRUE}.
#' @param outputList
#'        True if \emph{user wants to get the output list of CNA-driven differentially expressed genes in csv format}.
#'        \strong{Default value is TRUE}.
#' @param output_GSEA
#'        True if \emph{user wants to get the output list of CNA-driven differentially expressed genes in GSEA format}.
#'        \strong{Default value is FALSE}.
#' @return  list of CNA-driven differentially expressed genes
#' @examples
#' \dontrun{
#' CNAdrivenGene('CNAtoGeneList.rda', 'GeneExp.rda', False, 'FILE_SAMPLE_MAP', "C:/Users/Documents/Project/Mapping_file",
#' 7, 22, 0.25, 0.25, True, True, False)
#' }
#' @seealso
#' iGC-an integrated analysis package of Gene expression and Copy number alteration
#' @export
CNAdrivenGene <- function(CNAtoGeneList, GeneExp, probe = FALSE, sample.mapping.file,
    directory, map.loc.start, map.loc.end, gain.ratio, loss.ratio, tcga = TRUE,
    outputList = TRUE, output_GSEA = FALSE) {
    ...
    if (!tcga) {
        ...
        idx_tumor <- which(substr(colnames(CNAwholeGeneList_name), start = 14,
            stop = 14) == "0")
    ...
    ListGLratio <- data.frame(numGL, ListGL[, 2:4]/(ncol(CNAwholeGeneList_tumor) -
        1))
    names(ListGLratio) <- c("GeneList", "ratioGain", "ratioLoss", "ratioNormal")

    CNAwholeGeneListNum <- c()
    CNAwholeGeneListNum <- merge(CNAwholeGeneList_tumor, ListGLratio, by = "GeneList",
        sort = F)
    ...
    for (pp in 2:(ncol(thGlist_gexp) - 3)) {
        if (tcga) {
            idx_Gsam <- pmatch(substr(colnames(thGlist_gexp[pp]), start = 1, stop = 16),
                colnames(GeneExp))
        } else {
    ...
    for (qq in 2:(ncol(thLlist_gexp) - 3)) {
        if (tcga) {
            idx_Lsam <- pmatch(substr(colnames(thLlist_gexp[qq]), start = 1, stop = 16),
                colnames(GeneExp))
        } else {
