CNAtoGene <- function(file.pattern, directory, tcga=TRUE, cna.gain.threshold, cna.loss.threshold, col.sample, col.chrmosome, col.startloci, col.endloci, col.seg.mean, outputList=FALSE){
    		...
    		for (ii in 1:dim(cna_main)[1]){ 
       		...
       		idx_cn = cna_main[ii,idx_colchr]     
        		if (idx_cn == 23 || idx_cn == "X"){
            		...
            		idx_db = which(hg19DBNM[,2]==as.character(idx_cn))
			

			if (length(idx_db)>0){
        			idx_srt = head(which(cna_main[ii,idx_colstartloci]<hg19DBNM[idx_db[1]:tail(idx_db,1),4]),1)
        			idx_end = tail(which(cna_main[ii,idx_colendloci]>hg19DBNM[idx_db[1]:tail(idx_db,1),3]),1)
