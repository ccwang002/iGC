CNAdrivenGene <- function(CNAtoGeneList, GeneExp, probe=FALSE, sample.mapping.file, directory, map.loc.start, map.loc.end, gain.ratio, loss.ratio, tcga=TRUE, outputList=TRUE, output_GSEA=FALSE){
	...
	if (!tcga){
		...
		sampleName1 <- colnames(CNAwholeGeneList)
		idx_match <- match(substr(sampleName1,start=map.loc.start,stop=map.loc.end),substr(cna_map[,1],start=map.loc.start,stop=map.loc.end))
		sampleName2 <- cna_map[idx_match,2]
		sampleName <- rbind(sampleName1,as.character(sampleName2))
	...
	Gain_output <- data.frame(GeneList_Gain,meanGain,meanLoss,meanNormal,meanDiff_GnonG,thGlist_expmat[,(ncol(thGlist)-2):ncol(thGlist)],pValue_Gain,fdr_Gain)
	...
	Loss_output <- data.frame(GeneList_Loss,meanGain,meanLoss,meanNormal,meanDiff_LnonL,thLlist_expmat[,(ncol(thLlist)-2):ncol(thLlist)],pValue_Loss,fdr_Loss)
	...
	if (output_GSEA){
		...
		write.table(Gain_sorted_gsea, file = paste(path_map, "Gain_sorted_gsea.txt", sep="/"), row.names=F, sep="\t")	
		Loss_sorted_gsea <- Loss_sorted[,1:5]
		write.table(Loss_sorted_gsea, file = paste(path_map, "Loss_sorted_gsea.txt", sep="/"), row.names=F, sep="\t")	
		Both_GainLoss_sorted_gsea <- data.frame(Both_GainLoss_sorted[,1:5],Both_GainLoss_sorted[,12:15])
		write.table(GBoth_GainLoss_sorted_gsea, file = paste(path_map, "Both_GainLoss_sorted_gsea.txt", sep="/"), row.names=F, sep="\t")		
	}
}
